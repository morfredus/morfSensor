...
Copie autonome de la bibliothèque partagée de morfredus utilisée par morfSensor :

- `beacon/` — **morfBeacon** : supervision LAN (heartbeat UDP « je suis actif » +
  endpoint HTTP `/status`). morfSensor s'en sert pour s'annoncer sur le réseau.

> morfSensor n'embarque **que** morfBeacon (contrairement à ComponentHub, qui
