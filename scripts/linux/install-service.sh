...
# dossier FIXE (par defaut /opt/morfsensor), hors du clone git, puis installe et
# active le service « morfsensor » pointant la. Deplacer le depot (ou une synchro
# Syncthing) ne casse plus rien.
#
# Usage :
#   sudo ./scripts/linux/install-service.sh
#   sudo MS_APP_DIR=/opt/msensor ./scripts/linux/install-service.sh   # autre dossier
#   sudo ./scripts/linux/install-service.sh --uninstall

set -euo pipefail

SERVICE_NAME="morfsensor"
UNIT_DEST="/etc/systemd/system/$SERVICE_NAME.service"
APP_DIR="${MS_APP_DIR:-/opt/morfsensor}"
...
BIN="$BUILD_DIR/service/morfsensor"
...
mkdir -p "$APP_DIR"
install -m 0755 "$BIN" "$APP_DIR/morfsensor"

# La config n'est PAS ecrasee si elle existe deja (reglages locaux preserves).
if [[ ! -f "$APP_DIR/morfsensor.json" ]]; then
    install -m 0644 "$REPO_ROOT/config/morfsensor.example.json" "$APP_DIR/morfsensor.json"
    echo "Config initiale copiee : $APP_DIR/morfsensor.json (a adapter : port serie, capteurs)."
else
    echo "Config existante conservee : $APP_DIR/morfsensor.json"
fi
chown -R "$RUN_USER:$RUN_USER" "$APP_DIR"

# --- 4. Acces au port serie ----------------------------------------------
if ! id -nG "$RUN_USER" | grep -qw dialout; then
    usermod -aG dialout "$RUN_USER"
    echo "Utilisateur $RUN_USER ajoute au groupe 'dialout' (acces UART) — prend effet a la prochaine session."
fi

# --- 5. Installer et demarrer le service ---------------------------------
sed -e "s/__RUN_USER__/$RUN_USER/g" -e "s#__APP_DIR__#$APP_DIR#g" \
    "$SCRIPT_DIR/morfsensor.service" > "$UNIT_DEST"
chmod 0644 "$UNIT_DEST"
systemctl daemon-reload
systemctl enable --now "$SERVICE_NAME"
echo "Service '$SERVICE_NAME' installe (ExecStart -> $APP_DIR/morfsensor) et demarre."
